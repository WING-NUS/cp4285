...
## SASRec walkthrough: one next-item recommendation at a time
...
<div style="display:grid;grid-template-columns:minmax(0,1.18fr) minmax(320px,.82fr);gap:1.05em;align-items:stretch;font-size:.66em;line-height:1.18;">
  <div style="padding:.6em .72em;border:1px solid #9EBFDB;background:#EAF4FB;">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.48em;align-items:end;">
      <div class="cp-card cp-card--soft" style="padding:.38em;text-align:center;"><strong>museum<br>ticket</strong><br><span class="cp-text--muted" style="font-size:.76em;">earlier</span></div>
      <div class="cp-card cp-card--soft" style="padding:.38em;text-align:center;"><strong>vegetarian<br>café</strong><br><span class="cp-text--muted" style="font-size:.76em;">earlier</span></div>
      <div class="cp-card cp-card--prompt" style="padding:.38em;text-align:center;"><strong>late-night<br>halal food</strong><br><span style="font-size:.76em;">current query</span></div>
    </div>
    <div class="fragment" data-fragment-index="1" style="margin-top:.58em;padding:.42em .55em;border-top:2px solid #00796B;background:#fff;text-align:center;"><strong class="cp-text--teal">Step 1 · attention scores</strong><br><span class="cp-chip cp-chip--muted">0.08</span> <span class="cp-chip cp-chip--muted">0.22</span> <span class="cp-chip cp-chip--navy">0.70</span></div>
    <div class="fragment" data-fragment-index="2" style="margin-top:.42em;padding:.42em .55em;border-top:2px solid #00796B;background:#fff;text-align:center;"><strong class="cp-text--teal">Step 2 · causal softmax weights</strong><br><span class="cp-chip cp-chip--muted">8%</span> <span class="cp-chip cp-chip--muted">22%</span> <span class="cp-chip cp-chip--navy">70%</span></div>
    <div class="fragment" data-fragment-index="3" style="margin-top:.42em;padding:.42em .55em;border-top:2px solid #EF7C00;background:#fff;text-align:center;"><strong class="cp-text--orange">Step 3 · contextual state</strong><br>The model gives the late-night halal action the largest weight, while retaining the earlier context.</div>
  </div>
  <div style="padding:.6em .72em;border:1px solid #B9D6B3;background:#EEF8EA;">
    ...
    <div class="cp-card cp-card--soft" style="padding:.42em;"><strong>Session prefix</strong><br><span class="cp-text--muted" style="font-size:.86em;line-height:1.16;">museum ticket<br>→ vegetarian café<br>→ late-night halal food</span></div>
    <div class="fragment" data-fragment-index="4" style="margin-top:.48em;"><div class="cp-card cp-card--prompt" style="padding:.42em;"><strong>1. delivery-time check</strong><br><span class="cp-text--muted">highest-ranked next item</span></div></div>
    <div class="fragment" data-fragment-index="5" style="margin-top:.48em;"><div class="cp-card cp-card--soft" style="padding:.42em;"><strong>2. halal restaurant details</strong><br><span class="cp-text--muted">re-rank after the new observed action</span></div></div>
    <div class="fragment" data-fragment-index="6" style="margin-top:.48em;"><div class="cp-card cp-card--soft" style="padding:.42em;"><strong>3. order tracking</strong><br><span class="cp-text--muted">re-rank again with the longer prefix</span></div></div>
    <div class="fragment" data-fragment-index="7" style="margin-top:.48em;color:#5F6B7A;text-align:center;"><strong>Only past actions are visible.</strong></div>
  </div>
</div>
```

::: notes
Delivery: 4 minutes. Use this as a progressive build, following the user-provided CS4248 attention walkthrough pattern. Start with the three observed items. Reveal scores, then softmax weights, then the contextual state. Emphasize that the numbers are illustrative rather than learned values from a fitted model. Reveal each ranked next item as a new opportunity to re-run the causal sequence model on an extended prefix. Ask: “Which earlier action can this prediction inspect?” Answer: any earlier action in the displayed prefix, but never an action that occurs later.
...
<div class="cp-grid cp-grid--2" style="grid-template-columns:minmax(0,.82fr) minmax(0,1.18fr);align-items:center;margin-top:.62em;">
  <div class="cp-stack" style="font-size:.74em;line-height:1.22;">
    ...
    <div class="cp-card cp-card--prompt"><p class="cp-card__title">Causal masking applies the same rule within a sequence</p><p style="margin:0;">Before softmax, set scores for future positions to <strong>−∞</strong>. Each prediction can use its prefix, never a later action.</p></div>
  </div>
  <figure style="margin:0;text-align:center;">
